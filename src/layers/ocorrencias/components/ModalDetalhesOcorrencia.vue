<template>
  <UiModal v-model="isOpen" size="medium" :show-close="true">
      ...
      <div class="flex items-center gap-2.5 md:gap-3">
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex items-center gap-1.5 md:gap-2 mt-0.5">
            <span class="text-[10px] md:text-xs text-[var(--color-text-muted)]">#{{ ocorrencia?.id }}</span>
            <span class="w-1 h-1 rounded-full bg-[var(--color-text-muted)] opacity-50" />
            <span class="text-[10px] md:text-xs text-[var(--color-text-muted)] truncate">{{ ocorrencia?.fornecedor }}</span>
          </div>
        </div>
      </div>
    </template>
    ...
    <div class="flex flex-col gap-4 md:gap-5 -mx-4 md:-mx-6">
      <div v-if="ocorrencia?.proximoAtendimento" class="mx-4 md:mx-6 flex items-center gap-2 px-2.5 md:px-3 py-2 md:py-2.5 rounded-lg bg-[var(--color-primary-soft)] border border-[var(--color-primary-border)]">
        <Calendar class="w-3.5 h-3.5 md:w-4 md:h-4 text-[var(--color-primary)] flex-shrink-0" />
        <span class="text-xs md:text-sm font-medium text-[var(--color-primary)]">Próximo: {{ ocorrencia.proximoAtendimento }}</span>
      </div>

      <div class="flex items-center gap-4 md:gap-6 px-4 md:px-6 border-b border-[var(--color-border)]">
        <button
          v-for="tab in tabs"
          :key="tab.id"
          class="relative pb-2.5 md:pb-3 text-xs md:text-sm font-medium transition-colors"
          :class="abaAtiva === tab.id ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-muted)] hover:text-[var(--color-text)]'"
        ...
        <Transition name="fade" mode="out-in">
          <div v-if="abaAtiva === 'dados'" key="dados" class="flex flex-col gap-4 md:gap-5">
            <div class="flex flex-col gap-2.5 md:gap-3">
              <h3 class="text-[10px] md:text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider">Status</h3>
            ...
            <div class="flex flex-col gap-2.5 md:gap-3">
              <h3 class="text-[10px] md:text-xs font-semibold text-[var(--color-text-muted)] uppercase tracking-wider">Informações</h3>
              <div class="grid grid-cols-2 gap-1.5 md:gap-2">
                <DadoItem label="Data" :valor="ocorrencia?.dataCadastro || '—'" />
                <DadoItem label="Atendente" :valor="ocorrencia?.atendente || '—'" />
                <DadoItem label="Encaminhado para" :valor="ocorrencia?.encaminhadoPara || '—'" />
                <DadoItem label="Diagnosticado por" :valor="ocorrencia?.diagnosticadoPor || '—'" />
                <DadoItem label="Forma de atendimento" :valor="ocorrencia?.formaAtendimento || '—'" />
                <DadoItem label="Situação" :valor="getSituacaoLabel(ocorrencia?.situacao || '')" />
              </div>
            </div>
          </div>

          <div v-else key="historico" class="flex flex-col items-center justify-center py-8 md:py-10 text-[var(--color-text-muted)]">
            <div class="w-12 h-12 md:w-14 md:h-14 rounded-full bg-[var(--color-hover)] flex items-center justify-center mb-2.5 md:mb-3">
...
const getStatusBadgeClass = (statusValue: OcorrenciaStatus): string => {
  ...
  const map: Record<OcorrenciaStatus, string> = {
    pendente: "bg-[var(--color-danger-soft)] text-[var(--color-danger)] ring-1 ring-[var(--color-danger)]",
    acompanhamento: "bg-[var(--color-warning-soft)] text-[var(--color-warning)] ring-1 ring-[var(--color-warning)]",
    concluida: "bg-[var(--color-success-soft)] text-[var(--color-success)] ring-1 ring-[var(--color-success)]",
  };
