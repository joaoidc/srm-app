  ...
  <div class="flex flex-col gap-1.5">
    ...
    <div class="relative" ref="wrapperRef">
      <button
        ...
        :class="[
          disabled
            ? 'opacity-50 cursor-not-allowed is-disabled'
            : 'cursor-pointer',
    ...
    <span v-if="error" class="text-xs text-[var(--color-danger)]">{{
      error
    }}</span>
    <span v-else-if="hint" class="text-xs text-[var(--color-text-muted)]">{{
      hint
    }}</span>
  </div>
</template>
...
/* Add disabled style just in case */
